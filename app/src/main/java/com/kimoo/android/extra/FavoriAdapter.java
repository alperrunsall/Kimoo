...
public class FavoriAdapter extends RecyclerView.Adapter<FavoriAdapter.ViewHolder> {
    ...
    public FavoriAdapter(Context mContext,Activity mActivity, List<User> mUser) {
    ...
    @NonNull
    @Override
    public FavoriAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        View view = LayoutInflater.from(mContext).inflate(R.layout.favori_item,viewGroup,false);
        return new FavoriAdapter.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull final FavoriAdapter.ViewHolder viewHolder, int i) {
        fuser = FirebaseAuth.getInstance().getCurrentUser();
        final User user = mUser.get(i);

        fuser = FirebaseAuth.getInstance().getCurrentUser();
        ref = FirebaseDatabase.getInstance().getReference("usersF").child(fuser.getUid());
                ...
                if (!adminMi[0]) {
                    if (dataSnapshot.hasChild("rozetlerim")) {
                        if (dataSnapshot.child("rozet").getValue(String.class).equals("1")) {
                            viewHolder.rozetKismi.setVisibility(View.VISIBLE);
                        ...
                        if (dataSnapshot.child("rozet").getValue(String.class).equals("6")) {
                            ...
                            viewHolder.rozetKismi.setVisibility(View.GONE);
                    }
                    else
                        viewHolder.rozetKismi.setVisibility(View.GONE);
                }
                ...
                if (user.getGizlilik_ad().equals("0")) {
                    if (dataSnapshot.hasChild("begendiklerim")) {
                        if (dataSnapshot.child("begendiklerim").hasChild(fuser.getUid())) {
                            viewHolder.yas.setText(user.getDg());
                            ...
                            viewHolder.yas.setText(YasSansurle(user));
                        }
                    } else {
                        viewHolder.isim.setText(AdiSansurle(user));
                        viewHolder.yas.setText(YasSansurle(user));
        ...
        if(TaraActivity.tarada) {
            if (TaraActivity.favorilerde)
                ...
                viewHolder.favori.setVisibility(View.GONE);
            ...
            viewHolder.begeniIcon.setImageDrawable(mContext.getResources().getDrawable(R.drawable.ic_profil_begen_kirmizi));
            if (user.getBegeni_sayisi() == null)
                viewHolder.begenilme.setText("0");
            else
                viewHolder.begenilme.setText(user.getBegeni_sayisi());
                ...
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    if(dataSnapshot.hasChild("bulduklarim")){
                        if(dataSnapshot.child("bulduklarim").hasChild(user.getUid())) {
                            if (String.valueOf(dataSnapshot.child("bulduklarim").child(user.getUid()).child("kac_kez_gordum").getValue()) != null)
                                viewHolder.karsilasma.setText(String.valueOf(dataSnapshot.child("bulduklarim").child(user.getUid()).child("kac_kez_gordum").getValue()));
                            else
                                viewHolder.karsilasma.setText("0");
                        }
                        else
                            viewHolder.karsilasma.setText("0");
                    }
            ...
            viewHolder.favori.setVisibility(View.GONE);
            viewHolder.begeniIcon.setImageDrawable(mContext.getResources().getDrawable(R.drawable.ic_profil_begen2));
            DatabaseReference dref = FirebaseDatabase.getInstance().getReference("usersF").child(fuser.getUid());
            dref.addListenerForSingleValueEvent(new ValueEventListener() {
                ...
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    ...
                    if (dataSnapshot.hasChild("bulduklarim")) {
                        if (dataSnapshot.child("bulduklarim").hasChild(user.getUid()))
                            karsilasma = Integer.valueOf(dataSnapshot.child("bulduklarim").child(user.getUid()).child("kac_kez_gordum").getValue(String.class));
                        else
                            karsilasma = 0;
                    }
                    else
                        karsilasma = 0;
                    viewHolder.begenilme.setText(String.valueOf(dataSnapshot.child("begenenler").child(user.getUid()).child("kac_kez_begendim").getValue(Long.class)));
                    if(karsilasma > 0)
                        viewHolder.karsilasma.setText(String.valueOf(karsilasma));
                    else
                        viewHolder.karsilasma.setText("0");
    ...
    public class ViewHolder extends RecyclerView.ViewHolder{
        ...
        public TextView isim,yas,karsilasma,begenilme,yeni;
        public ImageView favori,profile_image,begeniIcon,rozet;
        public LinearLayout background;
        public ProgressBar pbar;
        public RelativeLayout rozetKismi;
        public ViewHolder(View itemView) {
            super(itemView);

            yeni = itemView.findViewById(R.id.yeni_yazisi);
            isim = itemView.findViewById(R.id.isim);
            yas = itemView.findViewById(R.id.yas);
            background = itemView.findViewById(R.id.background);
            karsilasma = itemView.findViewById(R.id.karsilasma_sayisi);
            begenilme = itemView.findViewById(R.id.begeni_sayisi_ulist);
            favori = itemView.findViewById(R.id.favori);
            pbar = itemView.findViewById(R.id.pbar);
            profile_image = itemView.findViewById(R.id.profile_image);
            begeniIcon = itemView.findViewById(R.id.begeni_icon);
            rozet = itemView.findViewById(R.id.rozet);
            rozetKismi = itemView.findViewById(R.id.rozetKismi);
        }
    }
}
